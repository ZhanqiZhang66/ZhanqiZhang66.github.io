...
Speculative decoding (also called speculative sampling) is one of the most elegant tricks in the LLM inference-acceleration toolbox. In one line: it lets a large model run at speeds that were previously unimaginable **without sacrificing any accuracy**.

## 1. The core pain point: why is large-model inference so slow?

Before the fix, we have to understand where the slowness comes from. LLMs generate text **autoregressively**: they must finish the first token before they can produce the second, and so on — like writing an essay one word at a time, never the first and third paragraph in parallel.
...
> An analogy: you need to move bricks (compute), but the bricks sit in a warehouse kilometers away (memory). The traditional approach: drive the truck to the warehouse → grab **one brick** → drive back → lay it → drive to the warehouse again → grab one more brick … No matter how fast your truck (GPU compute) is, all the time is spent on the road.

## 2. The intuition: a literary master and an intern

The core idea of speculative decoding is: **let a fast junior scribble ahead, and let the expert merely check the work.**

- **The large model (target model)** is a literary master — brilliant, writes flawless prose, but writes *very slowly* (every token requires deep deliberation and a full parameter haul).
- **The small model (draft model)** is an intern — mediocre, but writes *blazingly fast* (few parameters, cheap to move).
...
3. **Decision.**
   - If the master agrees that all 5 tokens match what he would have written, he **accepts them all** — 5 tokens generated in one round, a 5× speedup.
...
**Step 3 — Acceptance & rejection.** Compare the big model's distribution with the draft:
...
The mathematical guarantee: with a properly designed acceptance/rejection rule, the distribution of text produced by speculative decoding is **identical** to the distribution of the big model running on its own. The user gets exactly the result they would have gotten without the trick — just 2–3× faster.

## 5. Trade-offs

**Pros**

1. **Significant speedup** — typically 2× to 3×, and even more on structured tasks like code generation.
2. **No loss of quality** — no retraining of the big model required.
...
2. **Alignment requirement** — the "intern" and the "master" should write in similar styles. If the intern is too weak and is wrong every time, the master rejects everything and rewrites — *slower* than the original, because you also wasted time letting the intern guess.
...
- **Lookahead decoding.** No draft model at all. It uses the big model's own n-grams as drafts — having produced "San," it guesses "Francisco" is likely next.
...
1. **Problem:** the big model is slow because reading/writing memory is slow.
