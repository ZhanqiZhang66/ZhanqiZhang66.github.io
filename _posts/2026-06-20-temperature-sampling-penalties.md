---
layout: post
title: "The Temperature Knob"
subtitle: "How LLM sampling really works"
tags: [tutorials, llm, decoding, notes]
cover-img: /assets/img/blog/covers/temperature.svg
...
- **$T \to 0$.** The largest logit is exponentially amplified until it dominates everything. The distribution collapses to a single spike, which is exactly `argmax`, i.e. greedy decoding. Greedy is not "temperature-free"; it is the limit of temperature going to zero.
...
In practice $T$ usually lives somewhere around $0.2$ to $1.2$: lower for factual question answering, higher for creative writing. There is no universally "correct" value. A higher temperature raises the chance of sampling low-probability tokens and increases drift, but it does not *necessarily* produce gibberish. Whether the text survives depends on how flat the underlying distribution already is.

This points to one frequent source of confusion: **temperature scaling only matters when you sample.** If you take a pure greedy or `argmax` path, dividing all logits by a positive constant never changes the ordering, so the chosen token is identical. Where there is no randomness, the temperature knob spins in the void.
...
There is an important detail that is easy to miss. Penalties count **tokens**, not words and certainly not sentences. A word may be split into two tokens, and two semantically unrelated pieces of text may happen to share a high-frequency token. Both change what the penalty actually touches.
...
With $\alpha > 1$, positive logits shrink and negative logits grow more negative, pushing the probability of seen tokens down across the board. $\alpha = 1.0$ means no penalty; $1.1$ to $1.3$ is common. It does not distinguish one occurrence from ten, since appearing even once triggers the cut.
...
With $\beta > 0$, a token seen five times loses $5\beta$ and one seen once loses $\beta$. This is finer-grained than the repetition penalty, because the more something repeats, the harder it is pushed down, which suits the "high-frequency filler word keeps reappearing" failure mode.
...
Any token that has appeared once is reduced by $\gamma$, and no more. Its purpose is not to cure repetition but to **encourage new vocabulary**, which is useful when you want topical breadth, e.g. covering more subjects in a summary.
